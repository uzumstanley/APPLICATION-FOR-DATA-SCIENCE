...
figure(2);
subplot(121);
imshow(I);title('Disease Affected Leaf');
...
%% Classify the test leafy
species_disease = svmclassify(svmStructDisease,feat_disease)
%% Observe the results on the command window
title(['segmented and calssification is ',species_disease])

[Accuracy, Sensitivity, Fmeasure, Precision, MCC, Dice, Jaccard, Specitivity] = QualityEvaluation((img), medfilt2(img));
